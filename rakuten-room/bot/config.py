...
PERSONA_NG_KEYWORDS = [
    ...
    # 男性向け専用 (CEO 5/16 確定 / 2026-05-29 強化)
    # Codex 31回目 #2: 「ビジネスシューズ」単独はNG過剰 (レディース向けも存在) → 削除
    # masculine detection (item_auditor.py) が「メンズのみ表記」を自動 fail するため追加不要
    "メンズ専用", "紳士", "おじさん",
    # 介護関連商品名
...
PERSONA_GENRE_WEIGHTS = {
    ...
    # 2026-06-19 更新: 売上線分析(4-6月) → fashion/travel boost / interior/kitchen 削減
    "food":       3.0,   # 「全面推奨」CEO 指示・5-6月で gourmet 3件成約 (大五うなぎ/リンツ/ヒマワリ)
    "baby":       3.0,   # 0歳児ママ persona 必須・4-6月で Caraz 等成約
    "kids":       2.0,   # 入園準備等・売上7件成約 (popomi/devirock/京童工房/Hugmuu/トライブ/Booberg)
    "fashion":    1.5,   # ↑1.0→1.5 (welleg/靴ニシムラ 婦人靴2件・Booberg)
    "travel":     1.5,   # ↑新規 (楽天トラベル 5月2件成約・1件¥29,667高単価)
    "beauty":     1.5,   # ママ自分時間・サンテラボ成約
    "living":     1.0,
    "stationery": 1.0,
    "seasonal":   1.0,
    "interior":   0.7,   # ↓1.0→0.7 (roomy/あなろ/ゲキカグ/LOWYA 死蔵 click 多発)
    "kitchen":    0.7,   # ↓1.0→0.7 (鐵兎堂/くらしのもり/キッチンクレインズ 死蔵)
...
GENRE_SEARCH_KEYWORDS = {
    ...
    # 2026-05-29 CEO指示: 男服排除・新米ママペルソナ徹底
    # 変更: 「Tシャツ メンズ」→「Tシャツ レディース」/ 「ベルト 本革」→「アクセサリー レディース」
    # 2026-06-19: welleg/靴ニシムラ婦人靴2件成約の線から「パンプス/フラットシューズ/ブーツ」追加
    "fashion":   ["バッグ レディース 軽量", "スニーカー レディース", "ストール UV レディース", "帽子 レディース", "ワンピース 春",
                  "サンダル レディース", "Tシャツ レディース", "財布 コンパクト レディース", "日傘 折りたたみ", "アクセサリー レディース",
                  "パンプス レディース 痛くない", "フラットシューズ レディース", "ブーツ レディース 軽量"],
    "appliance": ["モバイルバッテリー 軽量", "ワイヤレスイヤホン", "加湿器 卓上", "LEDライト デスク", "充電器 USB",
                  "扇風機 首掛け", "電動歯ブラシ", "体重計 スマホ連携", "ドライヤー 速乾", "延長コード タワー"],
    # 2026-05-29: 「プロテイン おすすめ」→ 「プロテイン 女性 産後」でママ特化に
    # 2026-06-19: 大五うなぎ¥627報酬成約の線から「うなぎ蒲焼」「焼き芋」追加 (高単価ご褒美グルメ強化)
    "food":      ["コーヒー ドリップ", "ナッツ 素焼き", "お菓子 詰め合わせ", "紅茶 ギフト", "スイーツ お取り寄せ",
                  "プロテイン 女性 産後", "グラノーラ", "はちみつ 国産", "ドライフルーツ", "お茶 ティーバッグ",
                  "うなぎ 蒲焼 国産", "焼き芋 紅はるか", "ご褒美 スイーツ"],
    "kids":      ["知育玩具", "水筒 キッズ", "お弁当箱 子供", "入学準備", "絵本 人気",
                  "レゴ ブロック", "色鉛筆 セット", "リュック 子供", "プール バッグ", "夏休み 工作"],
    # 2026-05-29 CEO指示: 「ビジネス書」「参考書」「自己啓発」→ ママ・育児本・絵本に差し替え
    # 「英語 学習」「資格 テキスト」も新米ママ発信としてズレ → 絵本・育児に変更
    "book":      ["育児書 ベストセラー", "レシピ本 時短", "絵本 赤ちゃん", "絵本 セット", "知育 絵本",
                  "育児 本 おすすめ", "マンガ 全巻", "子育て 本", "図鑑 子供", "英語 絵本 子供"],
    ...
    # 2026-05-29: 「ボールペン 高級」「名刺入れ 革」→ ビジネス系でママ発信でない → かわいい文具に変更
    "stationery":["手帳 2026 かわいい", "ペンケース 大容量", "付箋 かわいい", "マスキングテープ かわいい",
                  "ノート A5", "ファイル クリア", "スタンプ かわいい", "マーカー 蛍光", "シール デコ", "メッセージカード かわいい"],
    "baby":      ["ベビー服 新生児", "おむつ パンパース", "抱っこ紐 軽量", "哺乳瓶 ガラス",
                  "ベビーカー コンパクト", "離乳食 食器", "おしりふき", "ベビーモニター", "チャイルドシート", "ガーゼ タオル"],
    # 2026-05-29 CEO指示: 「ランニングシューズ メンズ」→「レディース」/ ゴルフ・テニス・サイクリングはママ向けでない → 産後ケア・ヨガ系に差し替え
    "sports":    ["ヨガマット 厚手", "ランニングシューズ レディース", "ダンベル 可変式", "スポーツタオル",
                  "水着 レディース", "プロテインシェイカー", "ヨガウェア レディース", "産後 骨盤ベルト", "ストレッチポール", "トレーニングウェア レディース"],
