"""高速スキャナー v2: ヒットしやすいクエリ優先 + 0件即スキップ + 候補出し特化
...
def validate_query(query):
    ...
    if len(words) > 4:
        raise ValueError(f"CEO指示違反: 検索クエリが5語以上 ({len(words)}語)。")
...
def main():
    ...
    # 進捗管理ファイル（3状態記録）
    progress_file = data_dir / 'fast_scanner_progress.json'
    ...
    for cand in unique_queries:
        ...
        for item_id in item_ids[:5]:
            ...
            for yc in same_query_cands:
                ...
                print(f'  >>> {auc} +{pct}% | ${info["priceUsd"]} | {info["title"][:50]}')
                print(f'      vs Y{yc["price"]:,} | {yc["title"][:50]}')
                sys.stdout.flush()
