...
Usage:
    ...
    python slack_bridge.py watch-guardian              # watchの死活監視+自動再起動（別ウィンドウで常駐）
...
# watch self-heal 設定
# ============================================================
# watchが生存している証拠として定期更新するファイル
WATCH_HEARTBEAT_FILE       = DATA_DIR / "watch_heartbeat.txt"
# self-heal ログ・履歴
WATCH_SELF_HEAL_LOG        = Path(__file__).parent / "logs" / "watch_self_heal.log"
WATCH_SELF_HEAL_HISTORY    = Path(__file__).parent / "state" / "watch_self_heal_history.json"
# heartbeat を更新する間隔（秒）
SELF_HEAL_HEARTBEAT_INTERVAL  = 30
# heartbeatがこの秒数以上古ければ異常と判定
SELF_HEAL_STALE_THRESHOLD     = 600   # 10分
# guardianが状態チェックする間隔（秒）
GUARDIAN_CHECK_INTERVAL       = 60
# 30分以内にこの回数失敗したら自動再起動停止
...
def watch_channel(interval: int = 5):
    ...
    last_heartbeat_update = 0  # 起動直後に必ず1回書く
    ...
    while True:
        try:
            ...
            # --- heartbeat 更新（30秒ごと） ---
            if now - last_heartbeat_update >= SELF_HEAL_HEARTBEAT_INTERVAL:
                _update_watch_heartbeat()
...
# ============================================================
# watch self-heal ヘルパー関数
# ============================================================

def _update_watch_heartbeat():
    """watchが生存している証拠として現在時刻をファイルに書く（atomic write）"""
    try:
        DATA_DIR.mkdir(parents=True, exist_ok=True)
        tmp = WATCH_HEARTBEAT_FILE.with_suffix(".tmp")
...
def _get_heartbeat_age_sec() -> float:
    """heartbeatファイルの経過秒数を返す。ファイル不在は999999"""
    try:
        if not WATCH_HEARTBEAT_FILE.exists():
            return 999999.0
        ts = float(WATCH_HEARTBEAT_FILE.read_text(encoding="utf-8").strip())
        return time.time() - ts
    except Exception:
        return 999999.0


def _count_recent_ack_timeouts() -> int:
    """system_state.json の recent_history から ACK_TIMEOUT 連続回数を返す"""
    try:
        state = state_mgr.load()
        history = state.get("recent_history", [])
        count = 0
        for entry in history:
            if entry.get("error_type") == ErrorType.ACK_TIMEOUT:
                count += 1
            else:
                break  # 連続でなければそこで終わり
        return count
...
def _load_self_heal_history() -> list:
    """self-heal 実行履歴を読む（なければ空リスト）"""
    try:
        if WATCH_SELF_HEAL_HISTORY.exists():
            return json.loads(WATCH_SELF_HEAL_HISTORY.read_text(encoding="utf-8"))
    except Exception:
        pass
    return []


def _save_self_heal_history(history: list):
    """self-heal 実行履歴を atomic write で保存"""
    try:
        WATCH_SELF_HEAL_HISTORY.parent.mkdir(parents=True, exist_ok=True)
        tmp = WATCH_SELF_HEAL_HISTORY.with_suffix(".tmp")
        tmp.write_text(json.dumps(history, ensure_ascii=False, indent=2), encoding="utf-8")
        tmp.replace(WATCH_SELF_HEAL_HISTORY)
    except Exception as e:
        logger.warning(f"self-heal history保存失敗: {e}")


def _self_heal_log(line: str):
    """watch_self_heal.log に1行追記"""
    try:
        WATCH_SELF_HEAL_LOG.parent.mkdir(parents=True, exist_ok=True)
        ts = datetime.now(timezone(timedelta(hours=9))).strftime("%Y-%m-%d %H:%M:%S JST")
        with open(WATCH_SELF_HEAL_LOG, "a", encoding="utf-8") as f:
            f.write(f"[{ts}] {line}\n")
    except Exception:
        pass


def _is_loop_limit_exceeded(history: list) -> bool:
    """30分以内に SELF_HEAL_MAX_RETRIES_WINDOW 回失敗していれば True"""
    window_start = time.time() - SELF_HEAL_WINDOW_SEC
    recent_failures = [
        h for h in history
        if h.get("result") == "failed" and h.get("ts", 0) >= window_start
    ]
    return len(recent_failures) >= SELF_HEAL_MAX_RETRIES_WINDOW


def _try_restart_watch() -> bool:
    """
    ...
    try:
        ...
        if age < SELF_HEAL_HEARTBEAT_INTERVAL * 2:
            _self_heal_log("第1段階: 成功 (heartbeat更新確認)")
            return True
        _self_heal_log(f"第1段階: heartbeat未更新 age={age:.0f}s → 第2段階へ")
    except Exception as e:
        _self_heal_log(f"第1段階: 例外発生 {e} → 第2段階へ")

    # 第2段階: startup_all.bat
    bat_candidates = [
        Path(__file__).parent / "startup_all.bat",
        Path(__file__).parent / "ops" / "startup_all.bat",
        Path(__file__).parent / "rakuten-room" / "bot" / "scripts" / "startup_all.bat",
    ]
    bat_path = next((p for p in bat_candidates if p.exists()), None)
    if bat_path:
        try:
            logger.info(f"[self-heal] 第2段階: {bat_path} を実行")
            ...
            age = _get_heartbeat_age_sec()
            if age < SELF_HEAL_HEARTBEAT_INTERVAL * 2:
                _self_heal_log("第2段階: 成功")
                return True
            _self_heal_log(f"第2段階: heartbeat未更新 age={age:.0f}s → 失敗")
        except Exception as e:
            _self_heal_log(f"第2段階: 例外発生 {e}")
    else:
        _self_heal_log("第2段階: startup_all.bat が見つからない → 失敗")

    return False


def _send_self_heal_notification(sender: str, reason: str, restarted: bool, success: bool):
    """self-heal 結果を ai-bridge に通知する"""
    now_str = datetime.now(timezone(timedelta(hours=9))).strftime("%m/%d %H:%M JST")
    status = "✅ 復旧成功" if success else "❌ 復旧失敗（手動確認要）"
    restart_str = "実行" if restarted else "スキップ（ループ上限）"
    text = (
        f"[watch self-heal]\n"
        f"対象: {sender}\n"
        f"理由: {reason}\n"
        f"対応: watch 再起動 {restart_str}\n"
        f"結果: {status}\n"
        f"時刻: {now_str}"
    )
    try:
...
def watch_guardian(interval: int = GUARDIAN_CHECK_INTERVAL):
    ...
    python slack_bridge.py watch-guardian で起動。
    """
    sender = get_sender()
    logger.info(f"[guardian] 起動 sender={sender} check_interval={interval}s")
    _self_heal_log(f"guardian 起動 sender={sender}")
    print(f"[watch-guardian] 監視開始 (間隔: {interval}秒, sender: {sender})")
    ...
    while True:
        ...
        try:
            ...
            # --- 異常判定 ---
            reason = None
            if age >= SELF_HEAL_STALE_THRESHOLD:
                reason = f"heartbeat stale {age:.0f}s (>{SELF_HEAL_STALE_THRESHOLD}s)"
            elif ack_errors >= MAX_RETRIES + 1:
                reason = f"ACK_TIMEOUT {ack_errors}回連続"

            if reason is None:
                # 正常
                logger.debug(f"[guardian] 正常 heartbeat_age={age:.0f}s ack_errors={ack_errors}")
                continue

            # --- 異常検知 ---
            logger.warning(f"[guardian] 異常検知: {reason}")
            _self_heal_log(f"異常検知: {reason}")
            print(f"[watch-guardian] ⚠️  異常検知: {reason}")

            # --- ループ上限チェック ---
            if _is_loop_limit_exceeded(history):
                msg = f"30分以内に{SELF_HEAL_MAX_RETRIES_WINDOW}回失敗 → 自動再起動停止（手動確認必要）"
                logger.error(f"[guardian] {msg}")
                _self_heal_log(msg)
                _send_self_heal_notification(sender, reason, restarted=False, success=False)
                # 次の検知を遅らせる（無限通知防止）
                time.sleep(SELF_HEAL_WINDOW_SEC)
                continue
            ...
            _save_self_heal_history(history)

            success = _try_restart_watch()

            entry["result"] = "success" if success else "failed"
            entry["end_ts"] = time.time()
            _save_self_heal_history(history)
            _self_heal_log(f"再起動結果: {'success' if success else 'failed'}")
            _send_self_heal_notification(sender, reason, restarted=True, success=success)

        except Exception as e:
            logger.error(f"[guardian] チェックエラー: {e}")
            _self_heal_log(f"guardian チェックエラー: {e}")
...
def main():
    ...
    # watch-guardian
    wg_p = subparsers.add_parser("watch-guardian", help="watchプロセスを監視し異常時に自動再起動")
    wg_p.add_argument("--interval", type=int, default=GUARDIAN_CHECK_INTERVAL,
                      help=f"チェック間隔(秒, default={GUARDIAN_CHECK_INTERVAL})")
    ...
    elif args.command == "watch-guardian":
        watch_guardian(interval=args.interval)
